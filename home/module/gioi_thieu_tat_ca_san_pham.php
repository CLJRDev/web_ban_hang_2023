<div class="container-fluid d-flex">
  <div class="d-flex flex-column position-absolute">
    <div class="p-2 sidebar mt-3 d-flex flex-column">
  ...
  <div class="container-md">
    <div class="row">
      <div class='col-lg-12 h4 p-3 mt-3 font-weight-bold text-uppercase title'>Tất cả sản phẩm</div>
      <?php                       
        $page_index = 1;
        $page_length = 20;
        if(isset($_GET['pid']))
        ...
        $sql = "SELECT * FROM san_pham LIMIT {$page_start}, {$page_length}";
      ...
      <div class="col-md-12">
        <div class="pagination d-flex justify-content-center">
          <ul class="pagination">
            <?php
              $row_number = execute_query("SELECT COUNT(*) AS dem FROM san_pham WHERE trang_thai=1")[0]['dem'];
              $page_number = (int) $row_number / $page_length;
              if($row_number % $page_length != 0)
                $page_number++;
              for($i = 1; $i <= $page_number; $i++)
                echo "<li class='page-item'>
                  <a href='/web_ban_hang/home/gioi_thieu_tat_ca_san_pham.php?pid={$i}' class='page-link'>{$i}</a>
                </li>";
            ?>
          </ul>
        </div>
